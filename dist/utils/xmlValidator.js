 ...
 */
export async function parseMismoXml(filePath) {
    try {
 ...
 */
export function extractLoanData(parsed) {
    try {
        ...
        if (!dealSets)
            throw new Error('Missing DEAL_SETS element');
        const dealSet = dealSets.DEAL_SET?.[0];
        if (!dealSet)
            throw new Error('Missing DEAL_SET element');
        const deals = dealSet.DEALS?.[0];
        if (!deals)
            throw new Error('Missing DEALS element');
        const deal = deals.DEAL?.[0];
        if (!deal)
            throw new Error('Missing DEAL element');
        // Extract LOAN data
        const loans = deal.LOANS?.[0];
        if (!loans)
            throw new Error('Missing LOANS element');
        const loan = loans.LOAN?.[0];
        if (!loan)
            throw new Error('Missing LOAN element');
        ...
        if (!parties)
            throw new Error('Missing PARTIES element');
 ...
 */
export function extractBorrowerData(parties) {
    ...
    for (const party of partyList) {
        ...
        if (!roles)
            continue;
        const isBorrower = roles.some((role) => {
        ...
        if (isBorrower) {
            ...
            if (!individual)
                throw new Error('Borrower INDIVIDUAL element not found');
            const name = individual.NAME?.[0];
            if (!name)
                throw new Error('Borrower NAME element not found');
 ...
 */
export function mapLoanStatus(xmlStatus) {
